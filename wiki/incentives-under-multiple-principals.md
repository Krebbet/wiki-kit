...
**Provenance correction [2026-08-20].** This wiki now holds a Wilson primary text — the 1967 *Public Interest* essay "The Bureaucracy Problem" — and **it contains no version of this typology and no action-observability × outcome-observability framework at all.** The typology comes from Wilson's 1989 book *Bureaucracy*, which this wiki does **not** hold. So the wiki's only route to it remains Dixit's citation, and it should be attributed accordingly: **the axis is held second-hand.** Wilson's own 1967 observability claim is a *different* one — that among five mutually incompatible criteria applied to the same agency, the most observable (fiscal integrity) becomes the operative standard by default — and it is registered `rejected` at [[dimensions-of-institutional-variation]] D103 as likely subsumed by D25/D50/D51. See [[rational-actor-accounts-of-bureaucratic-behaviour]]. **Recording caveat.** Wilson's own scheme names the both-observable cell (production organisations); **Dixit's text as captured here names only procedural, craft and coping.** The fourth cell is inferred from the structure of the 2×2 and is not attributable to this source. Dixit also flags, in his footnote 4, that Wilson uses "output" for what economists call action or effort — a vocabulary trap worth remembering when reading the original.
