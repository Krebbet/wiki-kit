...
> **Scan-plane note (librarian, 2026-09-24).** The "~1.1 m" scan plane on this page is the **June capture rig** (a tripod carrying the stereo camera, `data/calib/rig_geometry.json`). The **prototype rover** scans at **0.2972 m** — sd 0.5 mm, seven independent down angles at open floor, drone-prototype `eda/EDA228-actuator-verification/FINDING.md` (2026-09-24; an earlier 0.337 m was wall hits read as floor). One conclusion flips: §"our RPLIDAR C1 sits at ~1.1 m, at the very top of Nikoohemat's 0.8–1.10 m door-centre band — a lucky coincidence" does **not** hold for the rover, which sits well **below** that band. The page's stronger argument is unaffected and now carries the weight: door and wall faces are **vertical**, so any plausible scan height cuts a ~2.03 m leaf, and height is only critical for the handle. Expect more floor clutter near an aperture at 0.3 m than at 1.1 m. Canonical statement and the tilt consequences: [[tilting-2d-lidar-multiplane-capture]] §CORRECTION.
