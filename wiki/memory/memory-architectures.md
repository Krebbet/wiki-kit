...
- [[claude-code-memory-ecosystem]] — 2026-05-23; practitioner landscape of Claude Code memory options; instruction/knowledge/episodic three-problem framing is a practitioner restatement of this page's taxonomy.
- [[claude-mem]] — 2026-05-23; community plugin; AI-compress extract/compress instance in the retrieval-augmented family (ChromaDB+SQLite, proactive SessionStart + PreToolUse:Read injection).
- [[graphiti]] — 2026-05-24; temporal knowledge-graph (Zep's OSS core); bi-temporal validity + automatic invalidation, beyond Mem0g.
- [[cognee]] — 2026-05-24; graph+vector "memory control plane"; deep Claude Code hook integration.
- [[supermemory]] — 2026-05-24; extract-pole engine (OSS plugins, hosted commercial core); cross-tool memory portability.
- [[memori]] — 2026-05-24; extract-pole layer (MemoriLabs) capturing agent execution + chat into 8 typed categories.
- [[claude-self-reflect]] — 2026-05-24; conversation-history indexer (verbatim-first, local SQLite+HNSW over `~/.claude`).
- [[mcp-memory-server]] — 2026-05-24; official MCP knowledge-graph reference (entities/relations/observations, keyword search).
...
- [[memory/evomembench]] — 2026-05-25; 2×2 scope×content benchmark; knowledge-vs-execution asymmetry evidence; long-context baselines beat extraction on knowledge, extraction retains lift on execution at constrained budgets.
...
[[memory/evomembench]] (arXiv 2605.18421, 2026-05-25) introduces a **2×2 evaluation axis** orthogonal to this page's five-family taxonomy: scope (in-episode / cross-episode) × content type (knowledge-oriented / execution-oriented). Its 15 agent-memory methods map onto families 2–5 here; all extraction-based methods share a DeepSeek-V3.2 backbone (numbers not cross-harness-comparable with GroupMemBench or LongMemEval). Two findings: on **knowledge tasks**, long-context LLM baselines (Gemini-3-Flash #1) and BM25 beat or match extraction-based systems (Mem0, A-MEM, MemOS), confirming GroupMemBench's direction on a different corpus type; on **execution tasks**, procedural memory methods (ReasoningBank/AWM; family 3–4 adjacent) and extraction-based methods retain a meaningful lift — +14.5 pp at 16K context, narrowing to ~+8 pp at 128K. This knowledge-vs-execution asymmetry is the cleanest current evidence for a regime boundary within family 2 (retrieval-augmented stores): extraction overhead may not be repaid for knowledge retrieval but *is* repaid for procedural/execution memory at constrained context budgets. Token cost: 2–5× baseline for memory-augmented methods. See [[conflicts/verbatim-vs-extracted-memory]] for implications.
