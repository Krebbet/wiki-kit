...
- **[[clawnet-readout]]** — editorial readout (2026-04-25, expanded 2026-04-26) on the ClawNet paper (Yang et al. 2026, HKUST / HKGAI, arXiv 2604.19211) as candidate infrastructure for the multi-party-with-divergent-interests subset of the lever inventory. Maps ClawNet's three governance primitives (identity binding / scoped authorization / action-level accountability) onto [[data-disruption-strategy-map|Tier 1 #1 (DSAR coordination)]], levers #14 / #16 (commitment device), #1 (CBI agent representation), #4 (group-buy). Identifies the obfuscation / ACA cluster as architecturally incompatible. Five risks specific to a consumer-counter-power deployment. **2026-04-26 expansion:** added empirical-capability evidence (D1 AgenticPay + D2 LLM-Deliberation — frontier LLMs negotiate but show universal buyer disadvantage + adversarial collapse), substrate-dependency analysis (D3 A2A protocol security gaps), and centralised-vs-decentralised comparison (G1 DID/VC alternative architecture). Cross-anchored on the new mechanism pages [[agent-mediated-negotiation-empirics]], [[agent-interop-protocols]], [[decentralized-agent-identity]].
- **[[mechanism-synthesis-readout]]** — editorial readout (2026-04-26): cross-product of the 10 new mechanism pages × the existing 29-lever inventory. **Eight novel builds** that compose 2+ mechanism pages onto existing levers (Federated Pricing Observatory, Verifiable Mandate-Bound DSAR Pipeline, Multi-Collective Adaptive-Pricing Disruption, Periphery-First Threshold Coordinator, Lever-Selection Decision Tool, Layered Cooperative Governance, Dual-Use Byzantine-Robust Observatory, Privacy Budget Marketplace). **Five candidate new lever avenues** not yet on the inventory. Hard limits the new mechanism set imposes (adaptive-seller recovery, disparate-impact externality, DP-as-firm-counter, LLM-negotiation buyer disadvantage, antitrust risk on lever #26, DID/VC issuer governance). Tier 1 / 2 / 3 build-portfolio recommendation. Five open strategic questions for follow-on work.
