...
**Candidate substrate (added 2026-04-25):** [[clawnet-readout|ClawNet]]'s identity-binding + dual-layer authorization + append-only audit log maps directly onto the four DSAR-platform requirements above (templating + collection + pseudonymisation + aggregation). Each member's identity agent issues DSARs with provable mandate; the per-action audit trail and pre-execution backup are the substrate properties Trail of Bits had to engineer specifically for [[markup-citizen-browser|the Markup Citizen Browser]] panel. Caveat: must be deployed under a [[platform-cooperatives|cooperative orchestration shell]], not the original cloud-vendor configuration, for the same reasons [[paypal-honey|PayPal/Honey]] pattern-matches as a failure mode.
...
| **6. DP-trained pricing model** | Seller adopts ε-LDP training, publicly framed as privacy protection | Neither legal nor technical — it is a *structural* suppression of collective-action leverage | **No tool-level mitigation.** Policy response: pre-emptive naming of "DP-trained pricing as ACA-suppression" (see [[obfuscation-strategic-readout|open strategic questions]]). See [[the-firms-view\|§2]]. |
| **7. Adaptive-pricing recovery (added 2026-04-26)** | Naive contextual pricing breaks under feature manipulation (linear regret Ω(T)); adaptive seller using two-phase explore-then-exploit recovers Õ(√T) regret by inferring buyer manipulation rule (per *Contextual Dynamic Pricing with Strategic Buyers*, arXiv 2307.04055). | ML-engineering, not legal — but more sophisticated than risk class 5: not signature filtering but learning the manipulation *function* and pricing accordingly | Faster-than-seller-phase rotation of manipulation strategy; collective coordination across diverse manipulation rules so the seller cannot invert a single rule. See [[strategic-classification]] + [[the-firms-view\|§3]]. |
| **8. Disparate-impact externality of operator response (added 2026-04-26)** | Firm tightens decision threshold τ in response to consumer manipulation; cost of manipulation rises for all positive individuals; the *increment* falls disproportionately on disadvantaged groups (FICO empirical anchor: cost ratio κ=2 → ~2.5× Black-white gap multiplier on Δτ; per Milli et al. 2019, FAccT). | Not platform enforcement — equity externality borne by uncoordinated populations | **No tool-level mitigation.** Pair obfuscation lever with collective-constraint mechanisms ([[noyb\|strategic litigation]], [[buyer-cartels-antitrust\|antitrust-aware framing]], [[regulatory-responses\|regulatory mobilisation]]) on firm response. See [[strategic-classification]] + [[obfuscation-strategic-readout]] §7 + [[the-firms-view\|§4]]. |

The critical observation: **levers L2 and L3 land in risk class 1** (pure consumer choice). They carry no platform-enforcement risk at all. Every other lever sits higher up the risk ladder. This argues strongly for prioritising L2 and L3 before obfuscation-type tools. **Risk classes 7 and 8 (added 2026-04-26) are *additional caveats* on the obfuscation lever cluster** — they do not change the obfuscation risk classification (still 2–4 depending on distribution model) but they bound the *durability* and *equity profile* of obfuscation-as-strategy.
...
3. **Timing coordination commitment device.** Is Kickstarter-style "I'll commit IFF N others commit" enough? Does it need legal commitment (prepaid deposit, enforceable contract)? [[possible-strategic-levers|Lever #16]] flags this as research-needed. **Partial answer surfaced 2026-04-25** via [[clawnet-readout|ClawNet readout]]: agent-mediated bilateral approval + append-only audit log produces verifiable per-individual mandate evidence — a *technical* commitment device, distinct from but compatible with legal commitment instruments. Open follow-on: empirically how much commitment-strength does a verifiable-audit substrate add over a Kickstarter-style trust-only device? Likely material for retaliation-vulnerable populations, less material for low-stakes coordination.
...
- [[data-cooperatives]]
- [[strategic-classification]] — risk-class 7 + 8 are anchored here
- [[the-firms-view]] — risk-class 2, 5, 6, 7, 8 all have firm-side framings catalogued here
- [[algorithmic-collective-action]] — Tier 1 #2 (collective-timing / demand-strike) has formal foundation here
- [[clawnet-readout]] — Tier 1 #1 (DSAR coordination) substrate
- [[buyer-cartels-antitrust]] — antitrust risk frame for the lever portfolio
- [[complex-contagion]] — network-topology constraints on Tier 1 #2 (collective timing) commitment-device design
