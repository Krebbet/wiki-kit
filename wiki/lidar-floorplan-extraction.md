...
> **Scan-plane note (librarian, 2026-09-24).** The "~1.1 m" scan plane on this page is the **June capture rig** (a tripod carrying the stereo camera, `data/calib/rig_geometry.json`). The **prototype rover** scans at **0.2972 m** — sd 0.5 mm, seven independent down angles at open floor, drone-prototype `eda/EDA228-actuator-verification/FINDING.md` (2026-09-24; an earlier 0.337 m was wall hits read as floor). Two §6.5 conclusions are platform-specific, not universal: "**1.1 m is the worst possible single height**" and the counter-certifies-false-free-space hazard (1.8–4.8 m² per kitchen) describe the June rig. At the rover's 0.2972 m the ray cuts the counter **front** instead of passing over the worktop. Likewise §"no published 2D person detector applies to our mount": at 0.2972 m the ankle/lower-leg detectors (DROW, DR-SPAAM) **are** in their trained regime. The ceiling-projection recommendation survives on its own merits. Canonical statement and the tilt consequences: [[tilting-2d-lidar-multiplane-capture]] §CORRECTION.
