...
- **Vacuum LDS salvage at ~$16.** Salvaged LDS/LiDAR modules from used robot vacuums (LDS-006, LDS-01RR/Xiaomi, Neato XV-11) are available as used spare parts for approximately $16 shipped. The kaiaai/LDS Arduino library (and associated kaiaai/awesome-2d-lidars list) provides ROS2 drivers for these modules; Roborock released the Cullinan project open-sourcing their own LDS design. This is the fastest and cheapest path to a working SLAM prototype but not appropriate for production products — use for development only [`raw/research/cheap-lidar/04-kaiaai-2d-lidar-list.md` — kaiaai/awesome-2d-lidars community list]. See [[cheap-lidar-pricing-guide]] for context in the full pricing tier analysis.
