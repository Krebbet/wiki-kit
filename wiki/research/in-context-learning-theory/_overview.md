...
| --- | --- | --- | --- |
| Induction heads | Content-addressable copy circuit | Sharp training-time phase change; fails without $\geq 2$ layers; copies rare tokens | Strong causal evidence in small models; explains the phase change at scale |
| ICL = GD | Mesa-optimised gradient descent in the forward pass | $K$ layers $\approx K$ SGD steps; out-of-distribution generalisation should track GD's; depth helps | Exactly matches optimal estimators on regression; explains why deeper TFs beat plain GD (curvature correction) |
...
| ICL = GD | Synthetic regression (LSA TF, $\leq 5$ layers) | Explicit weight construction matched by trained weights | Optimal in $N$ (=context length) for linear; $K$ layers = $K$ free GD steps | 500k SGD steps, fresh tasks |
...
| Function class | Synthetic regression (GPT-2 9.5M) | Behavioural matching of optimal estimators | $k = d$ for linear; $k \approx s \log d$ for sparse; $k \approx 100$ for trees | ~32M prompts, but 1k distinct fns suffices |
...
1. **Update structure should mirror the implicit ICL update.** GD-in-attention says the implicit update is rank-1 outer product $(Wx - y) x^\top$ with a meta-learned $\eta$ — i.e. LoRA-shaped, not full-weight. Single-sample fine-tunes that respect this structure should be most efficient.
