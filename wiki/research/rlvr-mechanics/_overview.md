...
**Update 2026-09-25 (weekly-brief).** [[context-grounding-mechanism-reuse]] extends the RL-as-selection-not-learning cluster below via an independent causal method (attention-head knockout + activation steering, not token-logit-rank analysis) and a new behavior domain (context grounding under knowledge conflict, not math reasoning). Its central surprise: DPO produces the *largest* behavioral change of any arm tested (near-ceiling grounding) yet shows the same 7–8/8 causal-head overlap with the base model as the small-effect GRPO arms — effect size and mechanism-reuse are decoupled. Steering the untrained base model alone along a pre-training-estimated direction recovers 35–40% of DPO's full-training gain with zero gradient steps, the cleanest single-paper corroboration yet of [[rethinking-rl-sparse-selection]]'s token-level reuse claim, now at the circuit level and across SFT/DPO/RLVR alike.

**Update 2026-09-18 (weekly-brief).** Two additions push this theme past pure policy-side mechanics: [[grpo-secretly-prm]] shows GRPO's implicit credit assignment already *is* a Monte-Carlo PRM — collapsing a distinction this wiki had tracked as two separate themes ([[../process-reward-models/_overview|process reward models]] vs. RLVR mechanics). [[ppo-critic-value-flattening]] is the theme's first entry attacking the *critic* rather than the policy objective, and its SP³O result (a repaired critic-based PPO beating GRPO) is in open tension with the "critic is optional" cross-cutting theme below and with [[../rl-optimizers/_overview]]'s critic-free lineage framing — not yet resolved.
