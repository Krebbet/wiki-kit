...
- **For the project's "reference-textbook in prompt" setting, is OPSD the natural drop-in?** OPSD collapses the two-model requirement and uses verified traces as privileged info. Closer to the project's target than RLT's two-model setup. Zhao et al.'s OPSD (arXiv:2601.18734) has no dedicated wiki page yet; it would warrant one if the project moves in this direction.
