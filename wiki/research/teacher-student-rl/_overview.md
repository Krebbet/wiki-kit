...
- [[privileged-info-opsd]] — AMPLE-MATH: builds a 6-view answer-matched benchmark to isolate what a genuine teacher reference adds over a reference-free control; finds the added value is small/model-dependent and reverses sign with the student's training-rollout mode — complicates both positions in [[../../conflicts/opsa-vs-opd-pattern-transfer]] rather than resolving it.
...
- [[uecr-grpo-entropy-calibrated-credit]] — UECR-GRPO unifies GRPO verifier reward + OPD teacher preference into one pre-normalization utility, then redistributes credit via an entropy-calibrated, sign-preserving token projection; diagnostic finding that raw teacher preference conflicts with verifier correctness on 39–51% of pairs bears directly on the OPSA-vs-OPD conflict.
- [[ier-gradient-reliability-opd]] — IER (information-efficiency ratio): Fisher-geometry signal-to-noise reliability axis for OPD token selection, orthogonal to usefulness; 0.1%–1% token budgets match/exceed full OPD when combined with usefulness selectors. Third independent mechanistic account for OPD's sparse-token sufficiency.
