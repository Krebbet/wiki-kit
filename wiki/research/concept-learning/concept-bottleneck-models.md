...
Koh, Nguyen, Tang, Mussmann, Pierson, Kim, Liang (Stanford / Google, ICML 2020). A *concept bottleneck model* (CBM) factors prediction as $x \rightarrow_g c \rightarrow_f y$ where $c \in \mathbb{R}^k$ is a vector of human-specified concepts (e.g. "joint space narrowing", "wing colour"). The model is trained on triples $(x, c, y)$ so that an intermediate layer of width $k$ aligns component-wise with the concepts. At test time the model reads off $\hat{c} = g(x)$, then predicts $\hat{y} = f(\hat{c})$. The bottleneck makes interpretation and *intervention* on individual concepts mechanically possible, and on OAI x-rays / CUB birds CBMs match end-to-end accuracy while exposing concept-level handles.

## Method

- Three training regimes: **independent** (train $g$ on $(x,c)$ and $f$ on $(c,y)$ separately, compose at test time); **sequential** (train $g$, then train $f$ on the predicted $\hat{g}(x)$); **joint** (single weighted loss $L_Y(f \circ g, y) + \lambda \Sigma L_{C_j}(g_j, c_j)$). $\lambda \rightarrow 0$ recovers a standard end-to-end model; $\lambda \rightarrow \infty$ recovers sequential.
- Architectural recipe is trivial: take any deep classifier, resize one intermediate layer to $k = \#\text{concepts}$, and add the per-concept loss on that layer. Concept annotations are needed only at training time.
- **Test-time intervention.** Replace $\hat{c}_j$ with the oracle's true $c_j$ for any subset of concepts, propagate through $f$. For classification (CUB) intervene on logits at the 5th/95th percentile of the training distribution to approximate a binary edit. Concepts can be grouped (e.g. "wing colour" as one query) so a single human action can correct multiple binary attributes.
- Robustness mechanism: because each concept is shared across many $y$ values, the $x \rightarrow c$ map sees concept-coherent distributional diversity that the spurious-correlation-driven $x \rightarrow y$ map does not.
...
- **Test-time intervention.** OAI: querying just 2 of 10 concepts cuts RMSE from >0.4 to $\approx 0.3$. CUB: large gains after intervening on several concept groups.
- **Intervenability $\leftrightarrow$ training regime.** Independent bottleneck has the worst pre-intervention error but the best post-intervention error — because $f$ was trained on true $c$, the test-time substitution is in-distribution. Joint bottleneck with $\lambda$ too small fails to intervene at all (control: replacing $\hat{c}$ with true $c$ *increases* error).
- **Robustness on TravelingBirds.** Under train/test background swaps: standard 0.627 error, bottleneck models 0.48–0.50.
- **Theory (linear regression).** Asymptotic excess error ratio bound: $(k/d \cdot \sigma^2_Y + \sigma^2_C) / (\sigma^2_Y + \sigma^2_C)$. Bottleneck wins when $k \ll d$ and concepts are less noisy than targets.
