...
1. **Code-augmented CoT MCTS rollouts.** The policy SLM emits each step as a one-line NL CoT *plus* the corresponding Python; only steps whose code executes are kept as MCTS children. Selection uses UCT with $Q(s) = q(s)/N(s)$; PPM provides initial $q$ once it exists, and terminal-guided back-prop updates $q(s_i)^k = q(s_i)^{k-1} + q(s_d)^k$ with $q(s_d) \in \{-1, +1\}$ from ground-truth answer match.
