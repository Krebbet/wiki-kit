...
- [[3po-parameter-exploration]] — Venkatkrishna, Daheim, Gurevych (INSAIT/UKP Darmstadt, arXiv:2608.09805). Parameter-space exploration for RLVR: sample policy weights from an IVON variational posterior instead of/alongside temperature scaling. Best variant (C3PO) beats GRPO + three action-space exploration baselines at near-identical FLOPs; produces net-fewer zero-advantage groups during training — a third, prevention-based position in [[../../conflicts/mcpo-vs-dapo-mastered-prompts]].
...
**Tension (2026-09-18 weekly-brief, open).** [[../rlvr-mechanics/ppo-critic-value-flattening]] diagnoses *why* PPO's critic tends to fail (terminal-only reward ⇒ implicit variance penalty flattens the value function) and repairs it with sparse critic supervision (SP³O) rather than removing the critic — and in controlled comparisons SP³O beats both PPO *and* GRPO (+6.31pp math avg, +2.84pp OOD avg at 4B). This doesn't refute the critic-free trend generally (the paper's claim is narrower — PPO's critic was badly trained, not badly motivated), but it is a real counterexample to "critic-free is strictly better," not yet reconciled with the family-tree framing above.
