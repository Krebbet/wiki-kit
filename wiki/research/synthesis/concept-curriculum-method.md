...
| (ii) | **Bound the training process** | Three orthogonal bounds: **temporal** (max $K$ updates, early-stop on TestSet plateau), **spatial** ([[../rlvr-mechanics/rl-sparse-subnetwork]] mask restricting which weights move), **functional** (KL-to-base regulariser). Default inner loop: RLT reward $r^{SS} = \log \pi_S(A \mid Q, E, \text{Textbook})$ ([[../teacher-student-rl/sakana-rlt]]) optimised with Dr. GRPO ([[../rl-optimizers/dr-grpo]]) at small group $G$. Cheap baseline RL must beat: critique-FT on the same packet ([[../single-sample-rl-finetuning/critique-ft-one-problem]]). **Reward-design flag (2026-05-18):** evaluate **uPRM** ([[../process-reward-models/uprm]]) as the dense step-signal here — label-free (no step labels, no final answers; fits the textbook setting) and reward-hacking-robust under RL where supervised PRMs collapse. See the uPRM row in [[proposed-method]] §"Reward signal — beyond correctness or RLT alone" and the interventional alternative in [[parked-ideas]] P2. |
...
## Experimental tracking dimensions

*(Added 2026-05-29. Eleven dimensions to instrument across all experiments. Dims 1–6 were the initial set; dims 7–11 were added to cover failure modes the first set leaves undetected.)*

| # | Dimension | What to measure | Corpus tools / baseline | Notes |
|---|---|---|---|---|
| 1 | **Effective innerloop training** | Loss curve shape, gradient variance, reward trajectory, early-stop trigger sensitivity | RLT $r^{SS}$, Dr. GRPO, DAPO Dynamic Sampling | Foundation of §First experiment part (ii). Must pass before anything else gates. |
| 2 | **Teacher dataset construction** | Accuracy, diversity, and minimal-sufficiency of $\text{Packet}(C)$ at each $N$; quality of generated textbook, examples, sibling sets | KnowRL atomic-points + Constrained-Subset-Search, Ho diversity constraint, DOTS $p \approx 0.5$ calibration, contrast-sets as quality reference | Sweep $N \in \{1, 5, 20\}$. Diversity is load-bearing at small $N$ per [[../teacher-student-rl/ho-reasoning-teachers]]. |
| 3 | **Teacher autonomy** *(stretch)* | How autonomously $T$ generates textbook sections, exercise packets, and correction context without human curation; quality vs human-authored baseline | ConstitutionalAI, [[../teacher-student-rl/saha-teacher-explanations]], Sakana RLT think-tokens | Stretch goal. Gate: must have a human-curated baseline to compare against first. |
| 4 | **Concept stacking** | Performance on each concept's held-out eval after $k$ concepts trained sequentially; related-concept and unrelated-concept pairs tracked separately; specialisation vs general-capability tradeoff | §Second experiment 3-node DAG, [[skill-mix]] compositional retest | Format mandate is the *mitigation*; format drift (dim 11) is the *trigger* that says when to apply it. Track related vs unrelated separately — the forgetting profile will differ. |
| 5 | **Signal sparsity** | Fraction of parameters in the active mask at each training stage; how this shifts across concepts and steps | [[../rlvr-mechanics/rl-sparse-subnetwork]] (5–30% baseline, Fisher×magnitude mask) | Baseline already known: 5–30% of weights. Track per-concept deviation and trajectory across the stack. |
| 6 | **Signal overlap** | Parameter-space overlap of active masks across concept pairs | [[../rlvr-mechanics/rl-sparse-subnetwork]] (27% cross-domain / 67% same-domain empirical baseline) | This is *parameter-space* overlap, not behavioral interference — see dim 9. Low overlap does not guarantee low behavioral interference; high overlap does not guarantee it either. |
| 7 | **Concept installation fidelity** | Generalisation to novel/perturbed variants vs surface pattern-matching; MDL compression score on related unseen items | [[../concept-evaluation/contrast-sets]], [[../concept-evaluation/gsm-symbolic]], [[../concept-evaluation/counterfactual-tasks]], [[../concept-evaluation/math-perturb]], [[../concept-evaluation/skill-mix]], MDL sibling test from [[../concept-learning/recursive-concept-evolution]] | Already in §First experiment part (iv). Key test: reward rises but MDL sibling test does not → pattern, not concept. Eval tooling is ready. |
| 8 | **Prior coverage** | $p_\gamma(x)$ before training for each target concept; position relative to BOLT's feasibility threshold | [[../rl-optimizers/bolt-kl-rlvr-boltzmann]] Theorem 7 ($N \gtrsim 1/p_\gamma$), [[../self-play/yue-rlvr-boundary]] | Pre-experiment diagnostic, not a training metric. Determines whether RLVR can install the concept at all or whether SFT-first is required to lift the support. Open: how to probe $p_\gamma$ cheaply without a full RL run. |
| 9 | **Behavioral interference** | Does training on concept $B$ degrade held-out eval on concept $A$? Build a cross-concept behavioral interference matrix | No direct corpus tool — construct from dim-7 eval sets re-run after each new concept | Distinct from dim 6. Even low parameter-overlap concepts can interfere behaviorally; even high-overlap concepts may not. This is the behavioral reading of the RL→RL forgetting frame in [[single-sample-concept-skeleton]] §"Catastrophic forgetting bound". |
| 10 | **Reward hacking rate** | Divergence between reward signal and independent behavioral eval (dim 7) across training steps | [[../process-reward-models/uprm]] as the hacking-robust foil; supervised PRM collapses <50 iters on Qwen2.5-Math-7B | If reward rises and fidelity (dim 7) does not track, hacking is occurring. Track this per training run, not just at saturation. uPRM is the hacking-resistant alternative to baseline when this is suspected. |
| 11 | **Format drift** | Degradation in baseline format and instruction-following behaviour per concept training round; token-length trajectory | Self-Rewarding LM length-blowup ($1092 \to 2552$ tokens across 3 iterations) as a warning-signal calibration | Metric that triggers format mandate resets (dim 4). Without this measurement, mandate resets are arbitrary; with it, they become evidence-gated. Track separately from concept-performance degradation. |

**Forgetting velocity note.** Dim 4 (concept stacking) and dim 11 (format drift) both track degradation, but at different speeds and with different remedies. Concept-on-concept forgetting happens inside the active 5–30% subnetwork (the shared-knob contention zone); format drift may involve the inert 70–95% via pretraining-regime norms. Track them as separate time series — they can decouple.
