...
| **P4** | **Principle decomposition** (optional) — decompose each concept into 5–10 natural-language sub-criteria; evaluate each rollout along every principle; reward is a vector. **VPO objective flagged 2026-05-27** — when sub-principles are non-collinear on-policy ($\bar\rho < {\sim}0.8$), replace fixed-weight scalarization with the VPO set-level objective: sample $K$ vectors $w^{(k)} \sim \text{Dir}(1)$ per rollout-group, compute $\hat{R}(S^{(g)}) = \frac{1}{K}\sum_k \max_{s \in S^{(g)}} w^{(k)\top}r(x,s)$, use as the GRPO advantage input. *Guard:* requires $G > 1$ rollout-sets and a best@k or search-augmented eval regime (pass@1 degrades by design). Run on-policy $\bar\rho$ diagnostic first. See [[parked-ideas]] P3 and [[../rl-optimizers/vpo]]. | [[../critique-self-correction/constitutional-ai]], [[../rl-optimizers/vpo]] | Higher per-sample signal density. Addressable per-axis gradient instead of a scalar. VPO provides the rigorous set-level objective; non-collinearity is the necessary condition. |
