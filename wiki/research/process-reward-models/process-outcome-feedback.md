...
## Claims
- Final-answer error: outcome-only $\approx$ process-based both without RM (23.5 vs 22.3) and with RM (16.6 vs 14.8). Outcome supervision is more label-efficient at this metric (1–4 tokens vs hundreds).
...
- ORM $\approx$ PRM emulator: ORM step predictions agree with PRM labels 85% vs only 77% with the ORM labels themselves (Fig. 4). Authors hypothesise it's easier to recognise correct steps than to internally simulate the final answer.
- Best system: SFT + ORM-RL + ORM rerank $\rightarrow$ 12.7 final-answer / 3.4 trace; SFT + PRM-RL + PRM rerank $\rightarrow$ 12.9 / 3.8 (Table 1).
- Selective prediction: abstain 30% $\rightarrow$ final-answer error 14.1 $\rightarrow$ 2.7 ($5\times$ reduction with PRM).
...
## Sample efficiency
- Process-supervision data: only 1560 solutions $\times$ ~6 steps = 9856 step labels — orders of magnitude smaller than PRM800K, yet yields the lowest trace error in the paper.
- Outcome supervision is the most label-cheap ($\leq 4$ tokens/question) but loses on trace metrics.
