...
- **2026-08-28 update:** [[../research/rlvr-mechanics/es-broader-reasoning-coverage-grpo|arXiv:2608.27351]] adds a **method-class scope challenge** distinct from the curriculum/entropy-preservation escapes above. Evolution Strategies (ES), trained purely from verifier scalar rewards on the same objective $F(\theta) = \mathbb{E}[r(x,y)]$ as GRPO, exceeds **base-model** Pass@16/32 in several settings (e.g. Qwen2.5-1.5B-Instruct average Pass@32: base 80.2 → ES 80.9; DeepSeek-R1-Distill-Qwen-1.5B math average Pass@32: base 77.4 → ES 78.9), with margins modest but consistently positive across most target-task averages (one held-out non-math average is flat-to-slightly-below base). This sits at the boundary of what Theorem C.1/Corollary C.2 forbid on a naive reading of "RLVR," but the proof mechanism does not obviously extend to ES: Theorem C.1 is stated for methods that sample trajectories from $\pi_\theta$ itself and backprop (REINFORCE, PPO, GRPO, DAPO, REINFORCE++); ES never samples from $\pi_\theta$ — it evaluates population members $\pi_{\theta + \sigma\epsilon_i}$ and updates via a reward-weighted average of perturbation directions, so a perturbed model can assign nonzero probability where the center model assigned zero, and the update can shift the center's own support outward. Best read as a **method-class boundary question, not a resolved counter-example**: is Theorem C.1's scope properly narrow (gradient/on-policy-sampling methods only, in which case ES is a genuine fourth escape channel alongside curriculum design, entropy preservation, and distillation), or does the "invisible leash" framing intend to cover verifier-reward RLVR broadly (in which case ES needs its own proof of an analogous bound, not yet supplied by either paper)? Open for a ruling — no paper has yet stated or tested a population-based-optimizer analogue of Theorem C.1.
...
- [[../research/rlvr-mechanics/es-broader-reasoning-coverage-grpo]] — Evolution Strategies exceeds base-model Pass@16/32; method-class scope challenge to Theorem C.1 (2026-08-28, open ruling needed)
