...
| [[hrm]] | Wang et al. (arXiv:2506.21734). Original Hierarchical Recurrent Model: 27M-param dual-timescale H/L architecture for symbolic reasoning tasks, 1000 training samples, no CoT tokens. Sudoku-Extreme 61.3%, ARC-AGI-1 52.0% (beaten by GRAM's stochastic extension). Foundational paper for [[hrm-text]] (language version) and [[gram-recursive-reasoning]] (stochastic extension). |
...
| [[magistral]] | Mistral AI (arXiv:2506.10910). Pure GRPO-based RL reasoning model (no external distillation); Magistral Medium reaches 73.6% AIME'24 pass@1 from 26.8% base. No-KL GRPO, Clip-Higher ε_high=0.26–0.28, async 3-pool infra with mid-flight NCCL updates (<5s), language-consistency reward. RL on text unexpectedly improves vision (+12% MMMU-Pro-Vision). Magistral Small (24B, Apache 2.0). |
| [[reinforcement-pretraining]] | Microsoft/PKU/THU (arXiv:2506.08007). RPT applies GRPO-based RL at pre-training stage: each context position is a RL episode; binary prefix-matching reward uses the text corpus as verifier (no labels). Power-law scaling (α≈0.28–0.35, R²≥0.96) across entropy difficulty splits; +3.3–4.1 pp next-token prediction accuracy; stronger downstream RL fine-tuning foundation vs standard CLM pre-training. |
...
| [[qwen-agentworld]] | Qwen/Alibaba (arXiv:2606.24597). First language world models for agentic environments: Qwen-AgentWorld-35B-A3B and 397B-A17B simulate 7 agentic domains (MCP/Search/Terminal/SE/Android/Web/OS) via long-CoT reasoning. Three-stage training (CPT → SFT → RL with hybrid rubric+rule rewards); introduces AgentWorldBench. As decoupled simulator: synthetic RL surpasses real-environment training; as unified warmup: improves downstream across 7 agentic benchmarks. Code + weights released. |
...
| [[conflicts/sparse-policy-selection-vs-gradient-cancellation]] | Five-way mechanistic debate on why RLVR works. A: [[reasonmaxxer]] (sparse 1–4% high-entropy tokens drive gains). B: [[token-gradient-cancellation]] (gradient cancellation across shared tokens is the bottleneck). C: [[delta-token-credit]] (most tokens net-negative, discriminator reweighting). D: [[spurious-rewards-rlvr]] (GRPO clipping bias amplifies pretrained behaviors regardless of reward correctness; model-family-dependent). E: [[rlvr-incentivizes-reasoning]] (Pass@K confounded by guessing; CoT-Pass@K reveals persistent RLVR advantage; formal theorem proves GRPO incentivizes correct CoT under Logic Prior). [[thought-anchors]] corroborates sparse structure (behavioral layer). |
