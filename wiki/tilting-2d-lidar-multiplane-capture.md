...
The load-bearing calibration is the **tilt-axis-to-beam-origin lever arm**: a 4 cm arm at −26.6° displaces the sensor origin 1.8 cm, **3.6× our 0.5 cm registration accuracy**, so it is measured, never assumed — and a tilted plane is **not a height layer, it is a ramp**, z = 1.10 + x·tan τ, so "register each height as its own 2D map" is geometrically void.
...
Web-verified 2026-09-22; full list in *Sources*. Rig facts from `docs/wheel-calibration.md` §Procedure B (measured tilt travel), `data/calib/rig_geometry.json`, `src/drone/devices/tilt.py`, `src/drone/guard.py`, `src/drone/mapping/scanclean.py`, `docs/locked-map-office.md`, `docs/next-campaign-scoping.md`. All §6 geometry is computed here from h = 1.10 m and the measured travel; it is arithmetic, not a citation.
...
**The sweep geometry.** Tilting the head tilts the disc the beam sweeps, so each capture is still a **plane**, and everything in §6 follows from that one fact. The sweep's "vertical resolution" is not a sensor property at all — it is the angular step of the *actuator*, traded directly against sweep time (Kurt3D's 3.4 s ↔ 30 s span is exactly this trade).
...
The literature's answer to "how do I get these without a jig" is **self-calibration by cloud crispness**: sweep a static scene and choose the parameters that make the accumulated cloud sharpest, because a wrong lever arm smears a flat wall into a curved shell. Sheehan, Harrison & Newman do this for a rotating 3D laser with no external reference at all [sheehan-selfcal]; Alismail & Browning do it for the internal parameters of a spinning actuated lidar [alismail-spinning]. That is also the cheapest *check* available to us, and we have a second, even cheaper one that is specific to a floor-standing rig: the **floor line**. At tilt τ the floor returns form a straight line at exactly 1.10/tan|τ| — 2.20 m at −26.6° — and that distance moves ~9.6 cm per degree of tilt error. Drive to clear floor, command −26.6°, measure the line: that single number calibrates the tilt zero to a fraction of a degree.
...
| noise | can fuse N revolutions per angle | one pass per angle; no in-place averaging |
...
For this rig the choice is not close. We are **stationary at a station by construction**, we already fuse 5 revolutions per capture with a revolution-agreement check (and get ~0.1 cm wall rms out of it), and `devices/lidar.py` carries **one `time.monotonic()` stamp per revolution — there are no per-point timestamps in the stack**, nor any stream of servo angle at 10 Hz. Continuous nodding would mean building both. Stepped costs only dead time, and buys back the thing continuous nodding spends its whole calibration budget recovering: an exact, static pose per plane. **Stepped.** (Established practice: Kurt3D, the original stop-and-scan nodder, is stepped for the same reason.)
...
- **(c) Per-height 2D layers, each registered independently, then stacked.** Attractive because it reuses the whole existing 2D stack unchanged — and **geometrically void for tilt**: §6 shows a tilted plane is a ramp, not a constant-height slice (53 cm of height variation across one room at −15°). Constant-height layers can only be *sliced out of* an accumulated 3D cloud, which requires (a) or (b) first. The one way to get real layers directly is the mast's **Z-lift**, which translates the sensor without rotating it — but its total travel is unrecorded and the soft limit currently sits at 20 mm (`docs/lift-axis-bringup.md`), so it is not a height axis today.
...
- **The floor line.** See §7 — a down-tilted plane produces the straightest, densest "wall" in the scan, and it is the floor.
- **Grazing incidence on the floor.** At −26.6° the floor is struck at 26.6° incidence; the office lane is semi-specular hardwood plus a textured rubber mat ([[stop-and-scan-room-mapping]]), so expect drop-outs and long returns on the hardwood and honest returns on the mat. Untested here — treat the first ladder as the experiment that answers it.
...
**Scans per station.** Keep the existing 5 revolutions per angle (0.5 s at 10 Hz) with the revolution-agreement check. Seven angles ≈ 7 × (servo move + settle + 0.5 s capture); at ~1.5–2.5 s per step that is **10–18 s per station** against ~1–2 s today, and ~2–4 min added to a 13-station room. Judgement, not measured — time the first ladder.
...
- **The same cell, measured at several grazing angles, flips.** OctoMap documents exactly this trap: when a scanner sweeps a flat surface at a shallow angle, *a cell measured occupied in one scan is updated free in the next after the sensor rotates*; the fix is to **update each volume at most once per scan** [hornung-octomap]. A tilt ladder is a machine for generating that case — seven planes cut the same counter edge at seven incidences. Fuse the ladder per-angle, then merge; never let plane *k+1*'s ray carve what plane *k* just occupied.
...
2. **Measure the tilt-axis lever arm and the roll misalignment** — the one calibration that matters (§2). Face a flat wall at ~2 m, sweep the ladder, and solve for (ℓ_forward, ℓ_up, roll ρ, tilt zero) by making the wall plane fit; a 4 cm arm left unmodelled puts 1.8 cm into every scan at −26.6°, 3.6× our per-station registration error. Cross-check with the **floor-line ruler**: on clear floor at −26.6° the floor line must sit at 2.20 m, and it moves ~9.6 cm per degree of tilt error — a direct, cheap readout of the tilt zero.
...
Web-verified 2026-09-22 unless marked.
