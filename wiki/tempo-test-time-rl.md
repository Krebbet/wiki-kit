...
- [[ttpo-test-time-policy-opt]] — sibling test-time-RL-for-reasoning design; same problem space (Qwen reasoning models, competition math), different mechanism (TEMPO recalibrates a critic on *labeled* data, TTPO is fully label-free via majority-vote + asymmetric OPSD/GRPO routing). Together with [[u-opsd-unsupervised-self-distillation]] these form an RL/distillation-based test-time-adaptation cluster distinct from the architectural TTT cluster in [[test-time-training]].
