...
**Recommendation for the prototype (updated by EDA010 → corrected by EDA011 → reconciled with the 2026-06-05 floor-map research run).** ~~Start with Option B~~ ~~Prefer Option A (Unitree L1)~~ — both probes show passive-stereo floor-plan recovery is a **depth** problem (no texture → no/false disparity on walls), and EDA010's "L1 required" steer **overstated the case**. Two paths now, ranked by the research:

- **Hardware (the ground-truth answer): a 2D LiDAR + SLAM Toolbox + ROSE².** Note this is a *2D* LiDAR — **RPLiDAR S2 (~$200) or YDLIDAR X4 (~$70)** — not the 3D Unitree L1 ($249), whose 3D coverage is more than a floor plan needs. SLAM Toolbox gives clean walls from geometry (0.93 N-AUC indoor, 1.75 cm precision — [[slam-toolbox]]); [[room-segmentation-floor-plan]]'s **ROSE²** is a drop-in ROS node that segments any clean grid into rooms (73.3 IoU, no training). This is **the shortest path to a working 2D floor plan**. Accuracy ceiling for calibration: even Apple RoomPlan drifts ~37 cm over 6.45 m ([[apple-roomplan]]) — don't over-spec precision.
- **Software-only (no new hardware), to try first if staying passive: VPP (Virtual Pattern Projection).** Paints artificial texture onto blank walls using SGBM's own ~48% confident pixels as seeds, so the matcher gets disparity where it currently returns NaN — directly attacks the EDA011 root cause, no retraining, no hardware (Middlebury 19.2% → 6.65% error at 5% seeds). **This is the EDA012b candidate** ([[stereo-dense-reconstruction]] §VPP, [[passive-stereo-robustification]]). Learned monocular depth (Depth Anything v2 / Metric3D) and an active speckle projector (~$30) are the other passive cures.

Free-space carving alone (EDA011) already gives a usable *navigable-space* costmap even without clean walls — that part is solved for local obstacle avoidance. The open item is the **metric wall map**. ([[cheap-lidar-pricing-guide]], [[home-tidy-drone-prototype]])
