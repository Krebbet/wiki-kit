...
> **PROBED ON REAL DATA (2026-06-04, drone-prototype EDA010) — basic Option B is NOT usable here.** Fusing 122 keyframes of `room_sweep3` dense SGBM depth (19.8 M points) into a top-down occupancy grid, even with multi-view voting (≥4 distinct keyframes per cell to suppress radial smear), produced **no clean wall lines** — occupied cells hug the camera path (median 0.92 m, 74% within 1.5 m of the path) in a ~16×9 m room, i.e. near-field furniture reconstructs but the **far textureless walls are absent / lost in spurious-far-depth smear**. The blank-wall failure is real and dominant on this room. Caveat: tests the *basic* projection only (no WLS/confidence disparity filter, no free-space carving, no active stereo) — it shows the *cheap* Option B isn't a usable floor plan, not that passive stereo never can be. See `eda/EDA010-floorplan-probe/`.

**Recommendation for the prototype (updated by EDA010).** ~~Start with Option B~~ — the cheap SGBM projection does not give walls on real data (above). **Prefer Option A (Unitree L1 → SLAM Toolbox / Nav2 STVL)** for the floor plan; the L1 ($249 USD) is already on the buy list as the Phase-1 ground-robot navigation sensor. Pursue Option B only if staying passive, and only *with* the active speckle projector + disparity filtering + free-space carving — at which point it's a real project, not a free first cut. ([[cheap-lidar-pricing-guide]], [[home-tidy-drone-prototype]])
