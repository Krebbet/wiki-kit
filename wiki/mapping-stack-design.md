...
**Recommendation for the prototype (updated by EDA010 → EDA011 → 2026-06-05 floor-map research → 2026-06-05 action plan).** ~~Start with Option B~~ ~~Prefer Option A (Unitree L1)~~ — both probes show passive-stereo floor-plan recovery is a **depth** problem. Agreed action plan, in order:

**Step 1 — EDA012b: VPP (zero cost, try first).** Paints artificial texture onto blank walls using SGBM's own ~48% confident pixels as seeds, directly attacking the EDA011 root cause without hardware. Middlebury 19.2% → 6.65% error at 5% seeds; our 48% coverage gives far more than 5% seeds. If VPP recovers wall lines on prototype data, no new hardware is needed for months — move straight to room segmentation (ROSE²) and object library work. ([[stereo-dense-reconstruction]] §VPP, [[passive-stereo-robustification]])

**Step 2 — If VPP fails: $30 active speckle projector (before any LiDAR).** Attaches to the existing stereo camera, textures blank walls in hardware (~95%+ depth coverage indoors), solves the same root cause as VPP. Side effect: improves object shape depth for grasping. Total spend: ~$30. ([[passive-stereo-robustification]] §"Active stereo")

**Step 3 — After demonstrating the full tidy loop: Unitree L1 ($249, 3D).** Skip 2D-only LiDAR (YDLIDAR X4, RPLiDAR A1). Production robots running 2D LiDAR require a supplemental ToF/structured-light collision layer because the turret is too high for floor-level obstacles — negating the cost saving. The Unitree L1 at $249 gives 360°×90° 3D coverage (floor plan + vertical obstacle avoidance, chair legs, doorway clearance) in one sensor at 230 g, no supplemental layer needed. SLAM Toolbox gives clean walls (0.93 N-AUC indoor, 1.75 cm precision — [[slam-toolbox]]); ROSE² segments any clean grid into rooms (73.3 IoU, drop-in ROS node — [[room-segmentation-floor-plan]]). Accuracy ceiling: even Apple RoomPlan drifts ~37 cm over 6.45 m ([[apple-roomplan]]) — don't over-spec. ([[cheap-lidar-pricing-guide]])

**Note on LiDAR and object detection.** LiDAR is **not** the right tool for object detection or grasp planning — its point cloud is too sparse per-object (~hundreds of points on a mug at 1.5 m). Object detection uses camera + stereo depth (GDINO + MobileSAM + depth projection → object library); per-object dense shape uses GS2Mesh (10–20 stereo frames → TSDF mesh). Adding LiDAR does not change this stack. ([[object-fingerprint-memory]], [[stereo-dense-reconstruction]] §GS2Mesh)

Free-space carving alone (EDA011) already gives a usable *navigable-space* costmap — local obstacle avoidance is solved. The open item is the **metric wall map**. ([[home-tidy-drone-prototype]])
