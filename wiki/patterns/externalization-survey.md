...
## 2026-05-25 extension — two new complementary anchors

**[[patterns/code-as-agent-harness]]** (Shi et al., arXiv 2605.18747) is a complementary and orthogonal second vocabulary anchor for the same 2024–2026 literature. Where externalization-survey organises that body of work by *what gets moved out of weights* (memory / skills / protocols / harness — the weights→context→harness arc), code-as-agent-harness organises it by *code as the medium through which all of it runs*, using a three-layer Interface/Mechanisms/Scaling taxonomy and a Plan→Execute→Verify (PEV) loop as its structuring frame. The two surveys share a substantial citation overlap and arrive at compatible but non-identical taxonomies: externalization-survey foregrounds the *what* (cognitive-artifact theory, four externalizations), code-as-agent-harness foregrounds the *how* (code as execution substrate, PEV loop). Neither subsumes the other. They should be read together as dual lenses on the same empirical record.

**[[patterns/moss-production-self-evolution]]** (MOSS, arXiv 2605.22794) is a concrete production instance of the **§8.3 "self-evolving harnesses"** direction this survey names as emerging. MOSS operationalises that direction at deployment scale: agents propose source-level edits to the harness codebase, a controller validates and applies them in a live container via swap/rollback, and the harness permanently incorporates successful changes. On the weights→context→harness arc, MOSS sits at the maximally-externalised endpoint — the full cognitive environment is mutable source code under agent authorship. This places MOSS opposite [[patterns/sdar]] on the arc: SDAR internalises externalised skills back into weights for zero runtime footprint; MOSS externalises maximally and keeps the harness under continuous agent-driven revision.
...
- [[patterns/code-as-agent-harness]] — orthogonal second vocabulary anchor for the same literature: organises by code-as-medium (Interface/Mechanisms/Scaling + PEV loop) rather than by what-gets-externalised; read together with this page.
- [[patterns/moss-production-self-evolution]] — production instance of §8.3 "self-evolving harnesses": source-level harness rewriting + container swap/rollback; maximally-externalised endpoint of the weights→context→harness arc, opposite of [[patterns/sdar]].
