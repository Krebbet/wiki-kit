...
**Collective alternative — uniformity over rotation.** The cross-attribute inconsistency problem is structurally avoided by a different design: rather than each user rotating their own fingerprint, a shared cooperative infrastructure enforces the *same* fingerprint presentation for all members. This collapses entropy rather than randomising it — the approach Tor Browser uses for canvas (fixed 1000×1000px). This is documented in [[browser-fingerprinting|§ Relevance for consumer counter-power]] as **lever #11 (fingerprint parity network)**. The practical open question (from that page): the network must be large enough that the shared parity fingerprint blends into organic traffic — if the pool is small, "user who looks exactly like all other parity-network users" is itself an identifiable fingerprint. A shared-egress VPN pool that also enforces a uniform browser baseline is the cooperative-infrastructure implementation of this idea; it addresses the rotation problem by making individual distinction impossible rather than merely harder.
