...
| **Stochastic DP / RM (EMSR / fare-bucket)** | Williams 2018 (Cowles 3003U) | Airline pricing, hotel yield management | **No individual features** *(contested as of April 2026 — see Family 4 note below)*; capacity state + time + booking curve | Sales vs opportunity cost of capacity | Coordinated purchase-timing shifts break the booking-curve signal |
...
### Contested-claim flag (April 2026 — JetBlue inflection)

The "no individual identity features" finding above is reference-layer accurate as of the Williams 2018 source, which models monopoly-market US carriers under explicit model-assumption *"a single price to all customers"* per period. **The wiki marks this finding contested as of April 2026**, pending resolution of:

- The federal class-action complaint filed against JetBlue (April 22, 2026) alleging surveillance pricing using cached browsing data on commercial aviation. JetBlue's official statement denies the allegation; the complaint and FTC Chair Andrew Ferguson's directive to staff to *examine* whether new disclosure rules are needed (Senate Commerce Committee, April 23) place the question in active institutional dispute. *(Source: `weekly-2026-04-27/04-04-jetblue-surveillance-pricing-class-action.md` — Fortune, April 21 2026; cross-link [[consumer-facing-dynamic-pricing|JetBlue surveillance-pricing inflection event]].)*
- The 2026 multi-state legislative stack (NY / NJ / AZ / PA bills introduced; CA AG surveillance-pricing sweep) names airlines among target sectors; if any of those statutes or sweeps produces algorithm-level discovery, the no-identity-features framing on this page may need revisiting.

**May 2026 strengthening of the contested marker.** ClassAction.org's April 29 2026 coverage (`raw/research/weekly-2026-05-11/03-jetblue-class-action.md`) of *Phillips v. JetBlue Airways Corporation* (1:26-cv-02405) adds two facts the original Fortune coverage did not surface:

- The complaint names **PROS Holdings, Inc.** as the pricing algorithm vendor allegedly *"using an algorithm to set prices based on consumer behavior"* — and PROS is also among the eight recipients of the FTC's July 2024 surveillance-pricing 6(b) orders ([[surveillance-pricing-retail]]). The same vendor surfaces on both the regulatory inquiry and the civil complaint, with both sources treating PROS as a consumer-behavioural-input pricing engine. This is non-trivial *circumstantial* evidence against the "no individual identity features" reading: at minimum, PROS is alleged to consume per-user behavioural signals; whether that materially differs from the Williams-style aggregate-population-mix-inference framing is the load-bearing question, and it is an empirical question PROS could (but has not) resolved by algorithm disclosure.
- **FullStory, Inc.** (session-replay / behavioural-analytics vendor) is named alongside PROS as a data conduit. Session-replay data is *granular per-session per-user behavioural data*, not aggregate booking-curve velocity — which, if used as a pricing input, would directly contradict the no-individual-features reading.

**Wiki-position update.** The contested-claim marker remains *contested*, not *resolved-against-Williams*. The evidence that has moved the marker is (a) circumstantial (vendor identity overlap, vendor self-description) and (b) untested in discovery. No algorithm-level disclosure has happened. The "operational reading" below stands: continue to treat known-architecture RM systems (the Williams fare-class model) as identity-feature-free; treat *post-2020 industry trend* (Delta 3% → 20%, JetBlue's PROS deployment) as a separate sub-family whose identity-feature use is *open*. **For audit tooling**: the JetBlue case directly supports building a paired-session price-comparison detector that could produce *empirical* (rather than circumstantial) evidence of per-user pricing — see [[consumer-facing-dynamic-pricing#may-2026-build---phillips-v-jetblue-airways-corporation-classactionorg-coverage|tooling-hook framing on the consumer-facing-dynamic-pricing page]] and [[browser-fingerprinting|the case section there]].

**Operational reading.** The Williams 2018 / fare-class / single-price-per-period RM model remains the *standard* architecture documented in the academic / Cowles literature; the wiki retains that as the reference position. But individual-identity-based counter-strategies should not be ruled out as "structurally ineffective" against airline pricing as a category until the JetBlue case (or equivalent algorithm-disclosure event) resolves. **For [[strategies/data-disruption-strategy-map|the strategy map]]: continue to treat airline RM as identity-feature-free for *known-architecture* RM systems; flag *post-2020 industry trend* (Delta 3% → 20% AI-driven dynamic pricing per [[consumer-facing-dynamic-pricing|the airlines section]]) as a separate sub-family that may use individual signals — and that sub-family's identity-feature use is *open*, not settled.**
...
- [[strategic-classification]] — formal frame for individual-level feature manipulation against Family 1 / Family 3 pricing algorithms; documents the *Contextual Dynamic Pricing with Strategic Buyers* result (adaptive sellers recover Õ(√T) regret against feature manipulation — a critical caveat for this page's effectiveness claims)
- [[the-firms-view]] §3 — firm-side framing of robust pricing under strategic buyer manipulation
