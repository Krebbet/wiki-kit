...
11. **Stale or undated market data** — any page citing a Polymarket (or Kalshi) price, implied probability, or market-maker spread must include a date-stamped snapshot reference (e.g., `raw/markets/<slug>/<YYYY-MM-DD>.*`). Flag every price/probability claim that lacks a date, and flag any snapshot reference older than 30 days for a page presented as "current".
12. **Math without provenance** — any formula, constant, or coefficient (Kelly fraction, log-loss bound, calibration parameter, scoring-rule weight) must cite the source page/section it is derived in. Flag orphan equations and constants that appear without a `[[wiki-link]]` or `raw/` citation.
13. **Strategy pages missing scope** — pages describing a betting or modeling strategy must declare: (a) sample size or out-of-sample status, (b) the platforms or markets it was tested on, (c) liquidity assumptions or transaction-cost handling. Flag strategy pages missing any of these three.
14. **Calibration claims without scope** — any claim about market calibration ("Polymarket is well-calibrated", "markets beat polls", etc.) must cite sample size, time window, platform, and the calibration metric used. Flag scope-free calibration assertions.
15. **Specific-market pages missing resolution criteria** — pages on individual Polymarket or Kalshi markets must link to the captured resolution criteria text and any UMA dispute history (or note `(no disputes)` explicitly). Flag market pages without this.
<!-- /DOMAIN-SLOT -->
