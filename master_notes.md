...
**Scope:** kit
**Observation:** During the weekly-brief run, `tools.capture_pdf --src https://arxiv.org/abs/2608.03874 --engine marker` completed with no error and produced a plausible-looking markdown file (frontmatter, headings, 2 pages processed) — but the content was a rendering of the arXiv *abstract landing page* (nav chrome, "Submitted on", "View PDF" links, arXivLabs footer), not the paper body. This happens because the weasyprint HTML→PDF conversion path (added 2026-08-02, see the entry above) faithfully converts whatever HTML is at the given URL — and `/abs/` URLs are the landing page, not the paper. `tools.audit_captures` reported 0 issues (no broken images, not flagged as thin — the landing page has plenty of lines/links to pass the thinness heuristic). Caught only because a downstream ingest subagent noticed the summary would be abstract-only and flagged it. Fixed by re-running with the direct `/pdf/` URL (`https://arxiv.org/pdf/2608.03874`), which produced the full 6-page paper with tables.
**Implication:** Two complementary fixes:
...
**Recurrence (2026-09-20):** Happened again, 2 of 2 arXiv sources this run (`2609.04170`, `2609.20519`), both captured via `--src https://arxiv.org/abs/<id> --engine marker` — same silent landing-page-only result, again passing `audit_captures` with 0 issues, again caught only by a downstream ingest subagent noticing abstract-only content. Third occurrence of this exact failure mode with zero fixes landed. Worked around this run by re-capturing both via `capture_url --url https://arxiv.org/html/<id>v1` instead (full paper text, though inline figure images sometimes 404 and need stripping) rather than fighting the PDF path. Given three recurrences and a known reliable workaround, prioritize fix (1) above at the next `/harvest` — specifically auto-rewrite `/abs/` → `/pdf/` in `capture_pdf.py`, since the HTML-fallback-via-capture_url workaround is a viable but manual stopgap that unattended runs (like weekly-brief) can't discover on their own.
**Status:** open
