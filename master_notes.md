...
**Status:** open — **confirmed recurring 2026-08-28, fourth data point:** hit again on 4 of 5 captures this run (all used `/abs/` URLs; the 5th source was a non-arXiv blog URL, unaffected). Same signature: 174-178 lines each, `capture_method: "pdf"` in frontmatter despite being an HTML abstract render, audit's thin-check passed cleanly. This time it was NOT caught before dispatching ingest — one ingest subagent (source 01) surfaced it first by explicitly flagging the summary as abstract-only, which prompted checking the other 3 PDF captures (same signature confirmed by line count alone, no manual read needed) and re-capturing all 4 with `/pdf/` URLs before re-dispatching ingest. Cost: 4 wasted ingest-subagent calls this run (their stub summaries were discarded) plus the 4 re-capture + 4 re-ingest calls. The ingest-subagent's self-flagging behavior is now the de facto catch mechanism across two consecutive runs (2026-08-21 caught pre-ingest by manual read; 2026-08-28 caught post-ingest by subagent self-report) — this is a real but expensive safety net, not a substitute for the fix. Still not promoted via `/harvest` after 4 occurrences. Strongly recommend the auto-rewrite fix (`_resolve_source`: `arxiv.org/abs/<ID>` → `arxiv.org/pdf/<ID>`) lands before the next scheduled sweep.
...
**Status:** open — **confirmed recurring 2026-08-28:** second occurrence, identical error, same pymupdf fallback used for all 5 captures. `$PATH` (2026-08-07 note) also recurred a fourth time this run, workaround applied proactively. The pymupdf image-ref bug (2026-06-05 note) recurred a fourth time too — same `sed -i 's#raw/research/<topic>/assets/#./assets/#g'` workaround applied to all 4 PDF-derived captures, 0 audit issues after. All three issues remain unpromoted via `/harvest` after 3-4 confirmed occurrences each — worth prioritizing next harvest pass given the recurrence rate.

### 2026-09-18 — weasyprint, $PATH, and abs-vs-pdf-URL bugs all recur again; new self-inflicted asset-deletion gotcha

**Scope:** kit

**Observation:** Fourth data points, same run: (1) `--engine marker` failed with `No module named 'weasyprint'`, fell back to pymupdf for all 5 captures. (2) `poetry` not on `$PATH` in this shell — needed `export PATH="$HOME/.local/bin:$PATH"` before any `poetry run` call. (3) Candidate URLs from search subagents were all `arxiv.org/abs/<ID>` (as usual); ran the first pass of captures against `/abs/` unmodified and got the silent-success HTML-abstract-page signature again — this time caught immediately (not via the ingest subagent) because the line counts (172-180 each) matched the exact number documented in the 2026-08-28 master_notes entry for this same failure mode, which made it recognizable on sight. Re-captured all 5 with `/pdf/` URLs; got real bodies (1488-3057 lines each, 16-47 pages).
(4) New, self-inflicted: capture_pdf writes each capture's assets into `assets/<slug>/`, keyed only by slug, not by run-number-prefix. When cleaning up the bad `/abs/`-captured files after the successful `/pdf/`-recapture, `rm -rf assets/<slug>` for the 5 known-bad slugs also deleted the *good* just-recaptured images, because both runs used the same slug and therefore the same asset directory. Had to recapture 3 of the 5 a second time to recover the images. Lesson: when redoing a capture under the same slug, do the cleanup (delete old .md + old assets) *before* the new capture runs, never after — or use a distinct slug for the retry and rename at the end.

**Implication:** this is now the *fourth* confirmed occurrence of all three long-open bugs (weasyprint/marker, $PATH, abs-vs-pdf URL) in as many-or-more runs, each still unpromoted via `/harvest`. The abs-vs-pdf bug in particular keeps costing a full wasted capture pass every time it recurs (5 captures × ~1-2 min each) despite `capture_pdf.py`'s `_resolve_source` fix being a one-line rewrite. Recommend prioritizing these three for the next `/harvest` pass over new feature work. Separately, `audit_captures.py`'s "broken image refs" check has now twice caught a real problem (missing images from the deletion mistake above, not just the known path-anchor false-positive) — it's earning its keep as a mandatory gate, not just noise to route around.

**Status:** open

### 2026-09-25 — WebFetch on a JS-rendered aggregator page can fabricate structured content instead of erroring

**Scope:** kit

**Observation:** During the 2026-09-25 weekly-brief's source-survey step, a subagent ran WebFetch against `alphaxiv.org/explore/papers` (a JS-heavy single-page app) to check trending papers. Rather than returning an error, empty content, or an obviously-broken render, WebFetch returned what looked like a normal list of candidates — except the "arXiv IDs" were non-numeric, non-existent strings shaped like `2609.agents-covert-communication-test-time` (a slug, not an ID). The subagent caught this only because the format looked subtly wrong on inspection, not because anything failed loudly. A second, independent verification pass (direct WebSearch + arxiv.org lookup) on two other flagged-suspicious IDs from a different subagent this same run confirmed both were real — so the hallucination risk is real and distinct from ordinary skepticism about search-snippet accuracy; it's specifically a WebFetch-on-JS-SPA failure mode where the tool appears to synthesize page-shaped content when the actual JS-rendered DOM isn't retrievable.

**Implication:** Any workflow that WebFetches a JS-rendered page for structured data (paper IDs, prices, dates, etc.) should treat the result as unverified until cross-checked against a second, non-JS-rendered source (direct site search, a stable API, a community mirror) — especially for content that will be captured/cited/committed downstream. Worth a note in `/research` and `/weekly-brief`'s source-scanning steps: "if a source is JS-rendered and WebFetch is the only access path, treat its structured output (IDs, numbers, names) as unverified and re-check before use," not just "if WebFetch fails, fall back to WebSearch." The failure mode here isn't WebFetch failing — it's WebFetch silently succeeding with fabricated content.
