...
### 2026-08-20 — audit_captures.py mis-resolves repo-root-relative and absolute image refs
**Scope:** kit
**Observation:** During a `/lint` capture-fidelity pass, `tools/audit_captures.py` reported ~1000+ "broken image ref" issues across several topic dirs (`language-3d-scene`, `floorplan-hypothesis-methods`, `tidy-benchmarks`, `robot-arms-affordable`, most of `room-segmentation`/`consumer-robot-vacuum-mapping`/`humanoid-robot-perception`). Nearly all were false positives. Root cause: captures across the corpus write image refs in three incompatible styles — `./assets/...` (md-relative, works), absolute `/home/david/...` paths, and repo-root-relative `raw/research/<topic>/assets/...` (no leading `./` or `/`). The tool's regex (`_IMAGE_REF_RE`) has a negative lookahead that explicitly skips any ref starting with `/`, so absolute-path refs are silently never checked — a topic dir using that style (`ov3d-instance-seg`) reports "0 issues" but is actually unverified, not clean. For the repo-root-relative style, the tool does `(md.parent / ref).resolve()`; since `md.parent` is already `raw/research/<topic>/` and `ref` repeats that same prefix, the path gets double-joined and the (actually-existing) file is reported broken. Verified independently: `raw/research/language-3d-scene/assets/lerf-2303.09553/source.pdf-0-0.png` exists on disk but was flagged broken.
**Implication:** Fix `audit_captures.py` to normalize refs before existence-checking — strip a leading `raw/research/<topic>/` prefix if present, and resolve absolute-path refs against `topic_dir` rather than skipping them outright. Until fixed, any "broken ref" or "0 issues" result from this tool should be treated as unreliable and spot-checked by hand rather than trusted. The two genuine issues found this run (12 real cross-paper image collisions in `room-segmentation`, 1 in `humanoid-robot-perception`) were only trustworthy because collision-detection doesn't depend on the broken resolution path.
**Status:** open

### 2026-08-20 — /weekly-brief's documented cron install has no `git pull` before invocation
**Scope:** kit
**Observation:** While planning multi-machine sync for `/weekly-brief` (user wants it running unattended from a separate workstation while they continue interactive work on this wiki from a laptop), found that `.claude/commands/weekly-brief.md`'s "Local scheduling" section documents a cron line of the form `cd /path && git checkout <branch> && claude -p "/weekly-brief"` — no `git pull` between checkout and invocation. On any setup where more than one machine can push to the same topic branch (the documented norm: an interactive dev machine plus a cron machine), a stale checkout means the run works from old content and its own step-7 `git push` can fail non-fast-forward, silently dropping that week's radar sweep from the branch (the brief artefact still lands on disk / gets emailed, but nothing reaches origin). The skill's own failure-mode doc says "do not retry blindly, surface the error" for the push failure, but nothing prevented the *stale-run* in the first place.
**Implication:** Fixed in this session by adding `git pull --ff-only origin <branch>` between checkout and invocation in the documented cron line, with a note explaining why `--ff-only` (fails loudly on divergence rather than silently merging/rebasing over local state it doesn't understand). This is a one-line, low-risk fix worth promoting to every topic branch via `/harvest` — any wiki-kit user running weekly-brief from more than one machine hits this. Not yet harvested; flagging here per the harvest checkpoint.
**Status:** open
