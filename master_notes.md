...
### 2026-09-17 — capture_pdf on an arXiv /abs/ URL silently captures the abstract page, not the paper
**Scope:** kit
**Observation:** `capture_pdf --src https://arxiv.org/abs/2609.05907 --engine pymupdf` exited 0, reported "Processing... (0/1)/(1/1)" (a plausible-looking single-page run), and produced a 169-line markdown file that passed `audit_captures` cleanly on the "thin extraction" and PDF-pairing checks. The actual content was the arXiv **HTML abstract landing page** (nav chrome, abstract text, "View PDF" link, bibliographic-tools sidebar) — not the paper. `_resolve_source()` in `tools/capture_pdf.py` does a bare `httpx.get(src)` with no Content-Type or magic-byte check before writing the response body to `source.pdf` and handing it to the engine; PyMuPDF's format-sniffing opened the HTML as a 1-page document and extracted its visible text, which is indistinguishable from a genuine short single-page PDF under the audit's line/page-ratio heuristic. The one visible symptom was a single broken image ref (`source.pdf-0-0.png`, path-prefixed oddly) — easy to read as a cosmetic path bug rather than a signal that the wrong document was fetched entirely. Re-running against `https://arxiv.org/pdf/2609.05907` (the actual PDF endpoint) produced the correct 21-page, 904-line capture. Note also that when the correct 21-page PDF *was* captured, its image refs used a redundant `raw/research/<topic>/assets/...` prefix (repo-root-relative) instead of the `./assets/...` file-relative form `capture_url` and the marker engine both use — a second, smaller `_rewrite_image_refs`/pymupdf-engine inconsistency, worked around by hand this run (`sed` to strip the redundant prefix) rather than logged as its own finding since it's cosmetic once the right source is fetched.
**Implication:** `capture_pdf` should (a) validate the downloaded response's Content-Type is `application/pdf` (or check the `%PDF-` magic bytes) before treating it as a PDF, and fail loudly with a clear "got text/html, did you mean the /pdf/ URL?" message rather than silently succeeding; (b) for arxiv.org specifically, auto-normalize `/abs/<id>` → `/pdf/<id>` in `_resolve_source`, since `/research` and `/weekly-brief` instructions both tell agents to pass the abstract-page URL form (`arxiv.org/abs/...`) as `--src`, and every prior successful capture in this wiki's history was presumably against an already-correct `/pdf/` URL by luck or convention rather than by contract. This is exactly the kind of "audit says clean but content is wrong" failure that erodes trust in the audit step — worth a note in `research.md`/`weekly-brief.md` too: a capture that "succeeds" with a suspiciously generic-looking single-page result on a multi-page source is worth a manual spot-check, not just a clean audit run. Promote via `/harvest`.
**Status:** open
